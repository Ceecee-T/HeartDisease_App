...
# everything sits flat in the repo root, no subfolder
@st.cache_resource
def load_components(prefix):
    return {
        "preprocessor": joblib.load(f"{prefix}_preprocessor.pkl"),
        "model": joblib.load(f"{prefix}_final_model.pkl"),
        "threshold": joblib.load(f"{prefix}_threshold.pkl"),
        "selected_features": joblib.load(f"{prefix}_selected_features.pkl"),
        "background": joblib.load(f"{prefix}_background.pkl"),
    }


@st.cache_resource
def build_explainer(prefix, _model, _background):
    # treats the whole stacked ensemble as a black box, since it's not
    # a single tree model
    def predict_fn(data):
